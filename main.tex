...
\usepackage{graphicx}
\usepackage{float}
\usepackage{booktabs} % Para líneas tipográficas profesionales
\usepackage{enumitem}
\usepackage{xcolor} % Opcional, para resaltar palabras claves si lo deseas
...
\nota{\color{red}ACORDARSE DE COMPLETAR, QUITAR NOMBRES Y DIRECCIONES DE MAIL}
...
Los 427 productos distintos que la empresa produce se distribuyen actualmente en 204 tipos distintos de caja.

\begin{table}[htbp]
  \centering
  \caption{Rangos y dispersión de dimensiones internas de cajas.}
  \label{tab:dimensiones_internas}
  \begin{tabular}{l c c c}
    \toprule
    \textbf{Dimensión} & \textbf{Mínimo (mm)} & \textbf{Máximo (mm)} & \textbf{Rango (mm)} \\
    \midrule
    Alto  & 138,0 & 363,0 & 225,0 \\
    Ancho & 182,0 & 297,0 & 115,0 \\
    Largo & 375,0 & 395,0 &  20,0 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[htbp]
  \centering
  \caption{Volumen de productos y cajas enviadas por planta.}
  \label{tab:envios_planta}
  \begin{tabular}{l *{3}{c}}
    \toprule
    \textbf{Planta} & {\textbf{Productos Enviados}} & {\textbf{Cajas Enviadas}} & {\textbf{Cajas Requeridas}} \\
    \midrule
    Curitiba     & 22.676.999 & 99.777.709 & 22.676.999 \\
    Santiago     & 15.292.073 & 67.677.375 & 15.292.073 \\
    Buenos Aires &  8.896.511 & 28.325.687 &  8.896.511 \\
    Bakersfield  &  8.504.666 & 38.218.479 &  8.504.666 \\
    Monterrey    &  7.050.662 & 42.254.246 &  7.050.662 \\
    \midrule
    \textbf{Total} & 62420911 & 276253496 & 62420911 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{tiers_plantas.png}
    \caption{Distribución por tiers de descuento para las cajas utilizadas por planta.}
    \label{fig:placeholder}
\end{figure}
Mediante el gráfico se identifican dos hallazgos clave que respaldan lo mencionado anteriormente:

\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=*, itemsep=0.8em]
    \item \textbf{Distribución por plantas:} Las plantas de \textbf{Curitiba} y \textbf{Santiago} destacan por ser las que reciben la mayor diversidad de tipos de cajas. Por el contrario, \textbf{Monterrey} presenta el menor nivel de actividad, con una recepción considerablemente más baja.
    
    \item \textbf{Volumen y beneficios logísticos:} Un porcentaje relevante de los tipos de cajas enviados a Curitiba y Santiago supera las 500.000 unidades (o al menos alcanza las 100.000 unidades). Esto implica que dichas plantas califican para los \textbf{descuentos más altos en el costo de flete}, lo cual, sumado al gran volumen de productos que manejan, representa una ventaja logística significativa.
\end{enumerate}

\vspace{0.5em}
\noindent\textbf{Observaciones adicionales:}
\begin{itemize}[leftmargin=*, label=\small$\bullet$, itemsep=0.4em]
    \item Todas las plantas presentan una cantidad similar de tipos de cajas con volúmenes bajos (menos de 20.000 unidades).
    \item Ciertamente, Bakersfield tiene un número considerable de tipos de cajas con volúmenes altos, ubicados en los tiers 1 y 2.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{u_pallet_dist.png}
    \caption{Distribución del porcentaje de utilización de pallet para cada tipo de caja.}
    \label{fig:placeholder}
\end{figure}
